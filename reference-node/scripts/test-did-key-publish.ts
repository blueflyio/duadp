...
async function main() {
  ...
  try {
    ...
    console.log(`   Publish successful (Status: ${resp.status})`);
    
    const verifiedTier = result.verified_tier || result.trust_verification?.verified_tier || result.resource?.metadata?.trust_tier;
